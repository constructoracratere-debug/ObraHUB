    ...
    <div className="relative flex h-dvh overflow-hidden bg-[#050b14] text-slate-200">
          ...
          {!sidebarCollapsed && (
            <div className="mt-6 space-y-6 md:hidden lg:block">
              <section>
                ...
                {isLoadingProjects ? (
                  <p className="px-2 text-xs text-slate-500">Cargando proyectos…</p>
                ) : projects.length === 0 ? (
                  <button
                    type="button"
                    onClick={() => { setShowCreateProject(true); setProjectError(null); }}
                    className="mx-1 w-full rounded-xl border border-dashed border-blue-500/25 bg-blue-500/[0.04] px-3 py-2.5 text-left text-xs text-slate-300 transition hover:border-blue-400/40 hover:bg-blue-500/10"
                  >
                    <span className="block font-medium text-blue-300">+ Crea tu primer proyecto</span>
                    <span className="mt-0.5 block text-slate-500">Presupuestos, cronograma y diseño IA viven aquí</span>
                  </button>
                ) : (
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex min-h-0 flex-1 flex-col">
                ...
                <div className="w-full">
                  <section className="text-center">
                    <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/5 px-4 py-1.5 text-xs font-medium text-blue-300">
                      ...
                      Construction OS · Hecho en Colombia, diseñado para Latinoamérica
                    </div>
          ...
          {/* Mapa de obras — SOLO cuando hay proyectos que mostrar: un mapa
              vacío al 70% del viewport comunica nada y roba el protagonismo.
              Sin proyectos: CTA compacto que invita a crear el primero. */}
          {portfolio.length > 0 ? (
            <div className="relative isolate">
              <AmericasMap cards={portfolio} onOpen={(slug) => openProject(slug)} />
            </div>
          ) : (
            <div className="mt-2 flex flex-col items-center justify-between gap-4 rounded-2xl border border-dashed border-blue-500/25 bg-blue-500/[0.04] px-6 py-6 sm:flex-row">
              <div className="flex items-center gap-4">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-500/10 text-xl ring-1 ring-blue-500/20">🗺️</span>
                <div>
                  <p className="text-sm font-semibold text-white">Tu portafolio de obras en el mapa</p>
                  <p className="mt-0.5 text-xs text-slate-500">Cada proyecto aparece geolocalizado con su avance y alertas.</p>
                </div>
              </div>
              <button
                type="button"
...
en Latinoamérica
                ...
                <div className="w-full py-4 sm:py-8">
                  ...
                  <details className="group mt-8 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4 open:pb-5">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-blue-400/80">
                        👥 Miembros del proyecto
                      </p>
                      <span className="flex items-center gap-2 text-[11px] text-slate-500">
                        {members.length > 0 ? `${members.length} invitado${members.length > 1 ? "s" : ""}` : "Sin invitados"}
                        <svg className="h-3.5 w-3.5 transition group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                        </svg>
                      </span>
                    </summary>
                    ...
                    {members.length > 0 ? (
                      ...
                      <p className="mt-2 text-[11px] text-slate-600">Comparte el proyecto con tu equipo invitándolos por correo.</p>
                    )}
                  </details>
                </div>
              ) : showFolderDashboard ? (
