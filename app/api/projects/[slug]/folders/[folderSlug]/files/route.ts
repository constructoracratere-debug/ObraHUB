...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!ctx) return NextResponse.json({ error: "Folder not found" }, { status: 404 });

    const files = await listFiles(supabase, ctx.folderId);
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    for (const file of entries) {
      ...
      const storagePath = `${user.id}/${ctx.projectId}/${ctx.folderId}/${Date.now()}-${file.name}`;
...
/** DELETE — delete a file (Storage object + metadata). */
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Remove the binary from Storage.
